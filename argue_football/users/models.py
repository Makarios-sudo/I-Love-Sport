...
from argue_football.utilities.utils import OTP_MAX_TRY, BaseModelMixin, generate_id, phone_regex_check
...
class User(AbstractUser):
    ...
    name = models.CharField(_("Name of User"), blank=False, max_length=255, db_index=True)
    ...
    otp_max_try = models.CharField(
        _("OTP Max Try"), max_length=2, db_index=True, null=True, blank=True, default=OTP_MAX_TRY
    )
...
class Account(BaseModelMixin):
    ...
    first_name = models.CharField(_("First Name"), blank=True, null=True, max_length=255, db_index=True)
    last_name = models.CharField(_("Last Name"), blank=True, null=True, max_length=255, db_index=True)
    club_interests = models.ManyToManyField(ClubInterest, blank=True, related_name="clubs_interest")
    ...
    def new_request(self: "Account", sender_account: "Account", *args, **kwargs):
        return v2_models.FriendRequest.objects.get_or_create(sender=sender_account, receiver=self, status="PENDING")
