...
class ClubInterest(BaseModelMixin):
    name = models.CharField(
        _("club Name"), max_length=200, blank=True, null=True, db_index=True
    )
    league = models.CharField(
        _("club League"),  max_length=200, blank=True, null=True, db_index=True
    )
    thumbnail = models.JSONField(
        _("Other Details"), default=dict, blank=True, null=True
    )
...
class Post(BaseModelMixin):
    ...
    body = models.TextField(
        _("Body"), blank=False, null=False, db_index=True
    )
    thumbnail = models.JSONField(
        _("Thumbnail"), default=dict, blank=True, null=True, db_index=True
    )
    ...
    def likes(self, *args, **kwargs) -> int:
        ...
        return PostActivity.objects.filter(
            post=self, is_like__isnull = False
        ).count()
...
class PostActivity(BaseModelMixin):
    ...
    comment = models.TextField(
        _("Comment"), blank=True, null=True, db_index=True
    )
...
class ActivityFeedBack(BaseModelMixin):
    ...
    response = models.TextField(
        _("Response"), blank=True, null=True, db_index=True
    )
    is_like = models.BooleanField(
        _("Is_Like"), default=False, null=True, blank=True
    )
...
class Share(BaseModelMixin):
    ...
    via= models.CharField(
        ...
        default=SharingOptions.DIRECT, max_length=50,
        db_index=True,
    )
    others= models.CharField(
        _("status"),
        choices=OtherSharingOptions.choices, 
        default=OtherSharingOptions.WHATSAPP, max_length=50,
        db_index=True,
    )
    direct = models.ManyToManyField(
        "community.Friends",
        blank=True,
        db_index=True,
        related_name="direct_sharing"
    )
    community = models.ManyToManyField(
        "community.Communities",
        blank=True,
        db_index=True,
        related_name="community_sharing"
    )
