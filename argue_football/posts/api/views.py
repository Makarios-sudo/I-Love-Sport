...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        qs = make_distinct(
            ...
            comments_count=Count(
                "post_engagements", 
                filter=Q(post_engagements__comment__isnull=False),
                distinct=True),
            likes_count=Count(
                "post_engagements",
                filter=Q(post_engagements__is_like=True),
                distinct=True
            ),
            reposts_count=Count(
                "post_engagements",
                filter=Q(post_engagements__is_repost=True),
                distinct=True
            ),
            bookmarks_count=Count(
                "post_engagements",
                filter=Q(post_engagements__is_bookmark=True),
                distinct=True
            )
    ...
    def get_reposts(self, request, *args, **kwargs):
        ...
        return self.get_paginated_response(
            v2_serializers.PostActivitySerializer.BaseRetrieve(
                qs, many =True
            ).data
        )
    ...
    def repost_unrepost(self, request, *args, **kwargs):
        ...
        if post.owner == user or post.account == account:
            raise custom_exceptions.Forbidden(
                "You can not repost your own post."
            )
        ...
        reposted = v2_model.PostActivity.objects.create(
                owner=user, account=account, 
                post=post, is_repost=True
            )
        return Response({
            "repost": v2_serializers.PostSerializer.BaseRetrieve(
                post
            ).data,
    ...
    def bookmark_unbookmark(self, request, *args, **kwargs):
        ...
        bookmarked = v2_model.PostActivity.objects.create(
                owner=user, account=account, 
                post=post, is_bookmark = True
            )
        return Response({
    ...
    def like_unlike_post(self, request, *args, **kwargs):
        ...
        v2_model.PostActivity.objects.create(
            owner=user, post=post, account=account, is_like=True
        )    
    ...
    def comment(self, request, *args, **kwargs):
        ...
        if post.owner == user or post.account == account:
            raise custom_exceptions.Forbidden(
                "You can not comment on your own post."
            )
        
        instance = serializer.save(owner=user, account = account)
        return Response(
            v2_serializers.PostActivitySerializer.AddComment(
                instance
            ).data
        )
    ...
    def like_unlike_comment(self, request, *args, **kwargs):
        ...
        serializer = v2_serializers.ActivityFeedBackSerializer.LikeActivity(
            data=request.data
        )
        ...
        liked = make_distinct(
            v2_model.ActivityFeedBack.objects.filter(
                owner = user, account = account, 
                postactivity = comment, is_like =True
            )
        )
        ...
        v2_model.ActivityFeedBack.objects.create(
            owner = user, account = account, 
            postactivity = comment, is_like =True
        )
